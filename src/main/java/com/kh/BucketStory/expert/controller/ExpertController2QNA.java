...
public class ExpertController2QNA {
	...
	public void insertMQnAjax(HttpSession session, HttpServletRequest request, HttpServletResponse response) {
		...
		if (result > 0) {resWriter(response, "ok");
	...
	public ModelAndView showQnAList(HttpSession session, 
		...
		int listCount = 0;	PageInfo pi = null;	ArrayList<adminQnA> list = null;
		...
		if (list != null) {
			mv.addObject("list", list).addObject("pi", pi).addObject("coId", coId).addObject("search",search).setViewName("hp_QnAList");
		} else {
	...
	public ModelAndView showMQnAList(HttpSession session, 
		...
		if(search.equals("all")) {
			...
			mv.addObject("list", list).addObject("pi", pi).addObject("userId", userId).addObject("search",search).setViewName("hp_MQnAList");
		} else {
			throw new ExpertException("QnA 내역 조회에 실패했습니다.");
		}
		return mv;
